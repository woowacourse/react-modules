...
describe("useCardIssuer", () => {
  test("사용자가 카드 발급사를 입력하면 유효성 검사를 통과한다", () => {
    ...
    expect(result.current.validateCardIssuer(result.current.cardIssuer).isValid).toBe(true);
  });
  ...
  test("사용자가 아무런 입력을 하지 않으면 유효성 검사에 실패한다", () => {
    ...
    expect(result.current.validateCardIssuer(result.current.cardIssuer).isValid).toBe(false);
    expect(result.current.validateCardIssuer(result.current.cardIssuer).errorMessage).toBe(
