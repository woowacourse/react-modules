...
describe('useExpiryDateValidation 테스트', () => {
  it('아직 아무 입력도 수행되지 않았을 때 true를 반환해야 함', () => {
    ...
    expect(result.current.validationResult.monthErrorMessage).toHaveLength(0);
    expect(result.current.validationResult.yearErrorMessage).toHaveLength(0);
  });
  ...
  it("현재 연도와 월보다 과거의 연도가 입력될 때 false와 '만료된 카드입니다.' 를 반환해야 함", () => {
    ...
    expect(result.current.validationResult.yearErrorMessage).toContain('만료된 카드입니다.');
  });
  ...
  it("현재 연도와 과거의 월이 입력될 때 false와 '만료된 카드입니다.' 를 반환해야 함", () => {
    ...
    expect(result.current.validationResult.isValidYear).toBe(true);
    expect(result.current.validationResult.monthErrorMessage).toContain('만료된 카드입니다.');
  });
  ...
  it("숫자가 아닌 입력이 들어올 때 false와 '숫자로 입력해주세요.' 를 반환해야 함", () => {
    ...
    expect(result.current.validationResult.isValidMonth).toBe(false);
    expect(result.current.validationResult.isValidYear).toBe(false);
    expect(result.current.validationResult.monthErrorMessage).toContain('숫자로 입력해주세요.');
    expect(result.current.validationResult.yearErrorMessage).toContain('숫자로 입력해주세요.');
  });
  ...
  it('길이가 잘못된 입력이 들어올 때 false와 2자로 입력해주세요. 를 반환해야 함', () => {
    ...
    expect(result.current.validationResult.isValidMonth).toBe(false);
    expect(result.current.validationResult.isValidYear).toBe(false);
    expect(result.current.validationResult.monthErrorMessage).toContain('2자로 입력해주세요.');
    expect(result.current.validationResult.yearErrorMessage).toContain('2자로 입력해주세요.');
  });
  ...
  it("잘못된 월 입력이 들어올 때 false와 '올바른 월을 입력해주세요 (01~12).'를 반환해야 함", () => {
    ...
    expect(result.current.validationResult.isValidMonth).toBe(false);
    expect(result.current.validationResult.isValidYear).toBe(true);
    expect(result.current.validationResult.monthErrorMessage).toContain('올바른 월을 입력해주세요 (01~12).');
    expect(result.current.validationResult.yearErrorMessage).toHaveLength(0);
  });
});
