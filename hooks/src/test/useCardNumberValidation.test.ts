...
describe('useCardNumberInputValidation 테스트', () => {
  it('아직 아무 입력도 수행되지 않았을 때 true를 반환해야 함', () => {
    ...
    expect(result.current.validationResults[0].errorMessage).toHaveLength(0);
  });
  ...
  it("숫자가 아닌 입력이 들어올 때 false와 '숫자를 입력해주세요.' 를 반환해야 함", () => {
    ...
    expect(result.current.validationResults[0].errorMessage).toContain('숫자를 입력해주세요.');
  });
  ...
  it("4자리가 아닌 입력이 들어올 때 false와 '4자리 숫자를 입력해주세요.' 를 반환해야 함", () => {
    ...
    expect(result.current.validationResults[0].isValid).toBe(false);
    expect(result.current.validationResults[0].errorMessage).toContain('4자리 숫자를 입력해주세요.');
  });
});
