...
test('사용자가 정상적인 월을 입력하면 에러가 발생하지 않는다.', async () => {
  ...
  act(() => handleExpiryDateChange(event as unknown as React.ChangeEvent<HTMLInputElement>));
...
test('사용자가 월에 문자를 입력하면 입력이 안된다.', async () => {
  ...
  act(() => {
    handleExpiryDateChange(event as unknown as React.ChangeEvent<HTMLInputElement>);
  });
...
test('사용자가 과거의 월을 입력했을 때 에러가 발생한다.', async () => {
  ...
  act(() => {
    result.current.handleExpiryDateChange(monthEvent as unknown as React.ChangeEvent<HTMLInputElement>);
  });
  rerender();
  act(() => {
    result.current.handleExpiryDateChange(yearEvent as unknown as React.ChangeEvent<HTMLInputElement>);
  });
