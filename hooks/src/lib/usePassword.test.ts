...
test('사용자가 정상적인 값을 입력하면 에러가 발생하지 않는다.', async () => {
  ...
  act(() => handlePasswordChange(event as unknown as React.ChangeEvent<HTMLInputElement>));
...
test('사용자가 한 글자를 입력하면 에러가 발생한다.', async () => {
  ...
  act(() => {
    handlePasswordChange(event as unknown as React.ChangeEvent<HTMLInputElement>);
  });
