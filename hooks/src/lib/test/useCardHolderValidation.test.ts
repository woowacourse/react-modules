...
describe("cardHolder에 입력 유효성 검증 커스텀 훅 테스트", () => {
    it("사용자 이름에 공백이 입력됐을경우 isError가 true가 되고 errorMessage가 '불필요한 공백이 포함되어 있습니다.'라는 값으로 재할당 된다.", async() => {
        ...
        expect(result.current.cardHolderValidation.errorMessage.cardHolder).toBe(errorMessage);
        expect(result.current.cardHolderValidation.isError[cardHolderName]).toBe(true);
