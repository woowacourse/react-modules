...
describe("유효성 검증", () => {
  ...
  test("사용자가 두 글자를 입력하면 에러 메시지를 표시한다.", () => {
    const { result } = renderHook(() => useCvcNumber());
    const { handleCvcNumberChange } = result.current;
    ...
    expect(result.current.errorMessage).toBe("3자리 숫자를 입력해 주세요.");
  });
});
