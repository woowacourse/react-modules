...
describe('useInput 테스트', () => {
  ...
  it('입력 값이 숫자가 아닐 때 해당 에러 메시지가 설정된다.', () => {
    const ERROR_MESSAGE = '숫자만 입력할 수 있습니다.';
  ...
  it('[입력 제한 유효성 검사] 입력 값이 숫자가 아닐 때 입력을 제한한다.', () => {
    ...
    const userInput = 'Hello';
    const { result } = renderHook(() => useInput({ initialValue: '', inputValidations, preventInputValidations }));
  ...
  it('[입력 제한 유효성 검사] 입력 값이 숫자가 아닐 때 해당 에러 메시지가 설정된다.', () => {
    const ERROR_MESSAGE = '숫자만 입력할 수 있습니다.';
