...
describe('useCardNumberInput custom hook 테스트', () => {
  ...
  it('카드 번호 4자리를 입력하면, 에러가 발생하지 않는다.', () => {
    act(() => {
      hookResult.result.current.handleCardNumbersChange('1234');
    });
    expect(hookResult.result.current.cardNumbersErrorMessage).toBe('');
    expect(hookResult.result.current.cardNumbers).toBe('1234');
  });

  it('문자가 입력되었을 때, 에러가 발생한다.', () => {
    act(() => {
      hookResult.result.current.handleCardNumbersChange('12a4');
    });
    expect(hookResult.result.current.cardNumbersErrorMessage).toBe('숫자만 입력 가능합니다.');
  });
});
